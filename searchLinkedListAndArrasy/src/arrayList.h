...
public:
	...
	unsigned int seqSearchIter(elemType e);
	unsigned int seqSearchRec(elemType e);
};
...
template <class elemType>
unsigned int arrayList<elemType>::seqSearchIter(elemType e) {
	for(int i=0; i<length; i++) {
		if(list[i] == e) {
			return i;
		}
...
template <class elemType>
unsigned int arrayList<elemType>::seqSearchRec(elemType e) {
//	for(int i=0; i<length; i++) {
	int head;
	if(head < 0 || head > length) head = 0;
		if(list[head] == e) {
			return list[head];
		}
		else {
